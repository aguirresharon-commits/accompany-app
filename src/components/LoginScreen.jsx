...
const LoginScreen = ({
  ...
  const [showCreatePasswordBlock, setShowCreatePasswordBlock] = useState(false)
  const [createPasswordIntent, setCreatePasswordIntent] = useState(false)
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      setLoading(true)
      setShowCreatePasswordBlock(false)
      try {
        ...
        setError(msg)
        if (err?.message?.includes('usa Google') || msg?.includes('Google')) {
          setShowCreatePasswordBlock(true)
        }
      } finally {
        setLoading(false)
      }
      ...
      <div className="login__inner">
        ...
        {!showEmailForm ? (
          <div className="login__actions">
            ...
            <button
              type="button"
              className="login__email-toggle-btn"
          ...
          <form className="login__form" onSubmit={handleSubmit} noValidate>
            ...
            {showCreatePasswordBlock && (
              <div className="login__create-password-block" role="region" aria-labelledby="create-password-block-title">
                <h2 id="create-password-block-title" className="login__create-password-title">
                  ¿Querés poder entrar sin Google?
                </h2>
                {!createPasswordIntent ? (
                  <>
                    <p className="login__create-password-text">
                      Creá una contraseña interna para tu cuenta. No modifica tu acceso con Google.
                    </p>
                    <button
                      type="button"
                      className="login__create-password-cta"
                      onClick={() => {
                        try {
                          window.localStorage?.setItem('control-want-set-password', '1')
                        } catch (_) {}
                        setCreatePasswordIntent(true)
                      }}
                    >
                      Crear contraseña
                    </button>
                  ...
                  <>
                    <p className="login__create-password-text">
                      Iniciá sesión con Google. Al volver, vas a poder crear tu contraseña.
                    </p>
                    <a
                      href={`${API_BASE}/api/auth/google`}
                      className="login__google-btn login__google-btn--in-block"
                      aria-label="Continuar con Google"
                    >
                      <svg className="login__google-icon" viewBox="0 0 24 24" aria-hidden>
                        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            ...
            <p className="login__create-account">
              <button
                type="button"
                className="login__create-account-link"
                onClick={() => { setShowEmailForm(false); setError(''); setShowCreatePasswordBlock(false); setCreatePasswordIntent(false); }}
              >
                Continuar con Google
              </button>
            </p>
